...
public class CryptoUtil {
    ...
    public static byte[] digest(byte[] input) {
        try {
            MessageDigest sha256MessageDigest = MessageDigest.getInstance("SHA-256",BouncyCastleProvider.PROVIDER_NAME);
    ...
    public static String formatPrivateKey(String privateKey) {
        ...
        return prefixPadding(privateKey,64,"0");
    }
    ...
    private static byte[] publicKeyHashFromPublicKey0(byte[] publicKey) {
        byte[] bytesPublicKeyHash = Ripemd160Util.digest(digest(publicKey));
    ...
    private static String base58AddressFromPublicKeyHash0(byte[] bytesPublicKeyHash) {
        ...
        //地址校验码(4个字节)
        byte[] bytesCheckCode = ByteUtil.copy(doubleDigest(bytesVersionAndPublicKeyHash), 0, 4);
